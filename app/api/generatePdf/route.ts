...
// Stub transcription (remplace par Whisper/OpenAI quand tu veux)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const audioBuffers = await Promise.all(audioPaths.map(downloadFileFromStorage));
    ...
    // 4) PDF
    const pdfBytes = await createSimplePdf('Bilan kinésithérapique', reportText);

    // 5) Upload
    const pdfPath = `reports/${consultationId}.pdf`;
    await uploadPdfToStorage(pdfPath, pdfBytes);

    // 6) URL signée (1h)
    const { data: signed, error: signErr } = await supabase.storage.from('reports').createSignedUrl(pdfPath, 60 * 60);
